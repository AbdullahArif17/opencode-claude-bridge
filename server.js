...
function loadConfig() {
  ...
  return {
    ...
    fallbackModels:
      Array.isArray(fileConfig.fallbackModels) && fileConfig.fallbackModels.length
...
function reasoningEffortToOpenAi(outputConfig) {
  const effort =
    outputConfig && typeof outputConfig === "object"
      ? outputConfig.effort
      : undefined;
...
async function callOpenCode(req, payload, upstreamContext) {
  ...
  const models =
    Array.isArray(CONFIG.fallbackModels) && CONFIG.fallbackModels.length
  ...
  for (const model of models) {
    if (upstreamContext.signal.aborted) {
      throw makeAbortError(upstreamContext);
    }
    ...
    console.log(`[ROTATION] Trying model: ${model}`);
    ...
    try {
      ...
      console.error(
        `[ROTATION] ${model} returned ${response.status}: ${text}`,
      );

      const error = new Error(
        `OpenCode Go returned ${response.status}: ${text}`,
      );
      ...
      if (
        ...
        console.log(
          `[ROTATION] Error with ${model}. Trying next model...`,
        );
        continue;
      }
  ...
  throw (
    lastError ||
    new Error("All configured OpenCode models failed.")
  );
}
